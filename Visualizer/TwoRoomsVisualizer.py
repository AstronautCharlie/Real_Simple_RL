...
class TwoRoomsVisualizer():
    ...
    # Creation methods
    # ----------------
    def create_grid(self, mdp, background=BLACK):
        """
        ...
        for col_idx, column in enumerate(range(1, mdp_width + 1, 1)):
            for row_idx, row in enumerate(range(mdp_height, 0, -1)):
                color = WHITE
                #print('on state', column, row)
                if not mdp.is_inside_rooms(GridWorldState(column, row)):
                    color = BLACK
    ...
    def create_abstract_gridworld_mdp(self, mdp, err_list=None):
        ...
        screen = self.create_grid(mdp.mdp)
        ...
        for col_idx, col in enumerate(range(1, mdp.get_width() + 1, 1)):
            for row_idx, row in enumerate(range(mdp.get_height(), 0, -1)):
                ...
                if mdp.mdp.is_inside_rooms(GridWorldState(col, row)):
                    ...
                    #print(abstr_state, abs_to_color)
                    try:
                        ...
                        if (tuple(abstr_state) in abs_to_color):
                            new_color = abs_to_color[abstr_state]
                        else:
                            new_color = rand_color.generate()
                            while (new_color in abs_to_color.values()):
                                new_color = rand_color.generate()
                            abs_to_color[tuple(abstr_state)] = new_color
    ...
    def create_abstract_mdp_from_file(self, ground_mdp, abstraction_file, key, err_list=None):
        """
        ...
        # Draw black-and-wide grid
        screen = self.create_grid(ground_mdp)
        rand_color = randomcolor.RandomColor()
        ...
        # print(k, value)
        abstr_mdp = AbstractMDP(ground_mdp, StateAbstraction(s_a, key[0]))
        ...
        for col_idx, col in enumerate(range(1, abstr_mdp.get_width() + 1, 1)):
            for row_idx, row in enumerate(range(abstr_mdp.get_height(), 0, -1)):
                #if (col, row) not in mdp.mdp.compute_walls():
                if abstr_mdp.mdp.is_inside_rooms(GridWorldState(col, row)):
                    ground_state = GridWorldState(col, row)
                    abstr_state_class = abstr_mdp.get_abstr_from_ground(ground_state)
                    abstr_state = abstr_state_class.data
                    if (abstr_state in abs_to_color):
                        new_color = abs_to_color[abstr_state]
                    else:
    ...
    def create_corruption_mdp(self, mdp, key, corrupt_abstr_file, error_file=None):
        """
        ...
        corr_abstr_mdp = AbstractMDP(mdp.copy(), StateAbstraction(s_a, key[0]))
        ...
        if error_file:
            corr_surface = self.draw_errors(mdp, corr_surface, key, error_file)

        return corr_surface

    def draw_errors(self, mdp, surface, key, error_file):
        ...
        for error in error_states:
            ...
            row = int((self.margin + self.cell_size) * (mdp.get_height() - error[0][1]) + self.margin + np.floor(self.cell_size / 2))
    ...
    # Roll-outs
    # ---------
    def draw_rollout(self, mdp, surface, rollout, color, agent_num):
        """
        ...
        while i < len(rollout) - 2:
            ...
            state_pos = (self.cell_size * (state[0] - 1) + (self.cell_size / 2) + int(3 * agent_num / 2),
                         self.cell_size * (mdp.get_height() - state[1]) + (self.cell_size / 2) + int(3 * agent_num / 2))
            next_state_pos = (self.cell_size * (next_state[0] - 1) + (self.cell_size / 2) + int(3 * agent_num / 2),
                         self.cell_size * (mdp.get_height() - next_state[1]) + (self.cell_size / 2) + int(3 * agent_num/ 2))
        ...
        #  resulted in a loop
        if (rollout[-1] not in mdp.goal_location) and (rollout[-2] not in mdp.goal_location):
            ...
            end_loc = (self.cell_size * (final_cell[0] - 1) + (self.cell_size / 2) + int(3 * agent_num / 2) + self.cell_size * margin[0],
                       self.cell_size * (mdp.get_height() - final_cell[1]) + (self.cell_size / 2) + int(3 * agent_num / 2) + self.cell_size * margin[1])
            pygame.draw.line(surface, BLACK, mid_loc, end_loc, 3)
        return surface

    def generate_true_abstract_rollout(self, mdp, key, policy_file, abstraction_file, agent_num):
        """
    ...
    def draw_true_ensemble_rollouts(self, mdp, surface, key, policy_file, abstraction_file, num_agents):
        """
        ...
        for agent_num in num_agents:
            ...
            #surface = self.draw_rollout(mdp, surface, rollout, color, i)
            surface = self.draw_rollout(mdp, surface, rollout, color, agent_num)
        return surface

    def generate_corrupt_abstract_rollout(self, mdp, key, policy_file, abstraction_file, agent_num):
        """
    ...
    def draw_corrupt_ensemble_rollouts(self, mdp, surface, key, policy_file, abstraction_file, num_agents):
        """
        Generate rollouts for each agent in the ensemble on the MDP matching the 'key' argument and display them
    ...
    # ---------------------
    # State value gradients
    # ---------------------
    def draw_state_value_gradient(self,
                                  mdp,
                                  key,
                                  agent_num,
                                  state_value_file,
        ...
        """
        # Draw basic grid
        surface = self.create_grid(mdp, background=background)
        ...
        for square in value_dict.keys():
            ...
            cell = pygame.Rect((self.margin + self.cell_size) * (square[0] - 1) + self.margin,
                               (self.margin + self.cell_size) * (mdp.get_height() - square[1]) + self.margin,
                               self.cell_size,
                               self.cell_size)
    ...
    def draw_misaggregations(self,
        ...
        for abstr_state in abstr_lookup_dict.keys():
            ...
            for err_state in abstr_lookup_dict[abstr_state]:
                ...
                row = int((self.margin + self.cell_size) * (mdp.get_height() - err_state[1]) + self.margin
                          + np.floor(self.cell_size / 2))
                radius = int(np.floor(self.cell_size / 3))
            ...
            for true_state in abstr_apply_dict[abstr_state]:
                ...
                top = int((self.margin + self.cell_size) * (mdp.get_height() - true_state[1]) + self.margin + np.floor(self.cell_size / 3))
    ...
    # Value Heatmap
    # -------------
    def create_value_heatmap(self, mdp, key, agent_num, state_value_file):
        """
        ...
        value_arr = np.empty((mdp.get_width(), mdp.get_height()))
        for i in range(1, mdp.get_width() + 1):
            for j in range(1, mdp.get_height() + 1):
        ...
        ax.set_xticks(0.5 + np.arange(mdp.get_width()))
        ax.set_yticks(0.5 + np.arange(mdp.get_height()))
        ax.set_xticklabels(1 + np.arange(mdp.get_width()))
        ax.set_yticklabels(1 + np.arange(mdp.get_height()))
        ...
        for i in range(mdp.get_width()):
            for j in range(mdp.get_height()):
                if mdp.is_inside_rooms(GridWorldState(i+1,j+1)):
